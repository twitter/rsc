...
class RichU64Long(l64: Long) {
  ...
  def toU64HexString: _root_.scala.Predef.String = (new RichU64ByteArray(toU64ByteArray)).toU64HexString
}
...
class RichU64ByteArray(bytes: Array[Byte]) {
  ...
  def toU64HexString: _root_.scala.Predef.String = bytes.map(deSign).map("%02x".format(_)).reduceLeft(_ + _)
  def toU64Long: _root_.scala.Long = new DataInputStream(new ByteArrayInputStream(bytes)).readLong
}
