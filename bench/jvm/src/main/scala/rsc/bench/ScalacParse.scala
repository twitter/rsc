...
import rsc.bench.ScalacParse._
import rsc.tests._
...
object ScalacParse {
  ...
  class BenchmarkState extends RscFixtures {
    ...
    val abstractFiles = re2sFiles.init.map(f => AbstractFile.getFile(f))
...
class ColdScalacParse extends ScalacParse {
  @Benchmark
...
class WarmScalacParse extends ScalacParse {
  @Benchmark
  def run(bs: BenchmarkState): Unit = {
    runImpl(bs)
  }
...
class HotScalacParse extends ScalacParse {
  @Benchmark
  def run(bs: BenchmarkState): Unit = {
    runImpl(bs)
  }
